...
// ─────────────────────────────────────────────────────────────────────────────
//  ПАТЧ ДЛЯ packages/web/src/web/components/layout.tsx
//
//  Заменить ЦЕЛИКОМ функцию `ContactForm` (последняя функция в файле, начинается
//  с комментария «// ── Contact form (reused across pages) ──…») на код ниже.
//  Остальной файл (SEO, Navbar, Footer, WhatsAppFloat, PageLayout) НЕ трогаем.
//
//  Что делает:
//   • Форма отправляет заявку НАПРЯМУЮ в EspoCRM (Lead Capture API).
//     Никакого сервера и serverless-функций — работает на Vercel как есть.
//   • Показывает статус: «Отправляем…» → экран «Заявка отправлена ✓».
//   • Кнопка WhatsApp осталась ОТДЕЛЬНЫМ каналом (звонки / свободные сообщения).
//   • Поля формы и их внешний вид не изменились.
//
//  Где взять URL: EspoCRM → Администрирование → Lead Capture → ваша запись.
//  В нём должны быть включены Payload Fields: First Name, Last Name,
//  Phone Number, Account Name, Description (иначе CRM проигнорирует данные).
// ─────────────────────────────────────────────────────────────────────────────

// URL приёмника EspoCRM Lead Capture. Это ключ «только на запись» —
// он может лишь создавать заявки и не даёт доступа к данным CRM,
// поэтому его безопасно держать во фронтенде.
const ESPO_LEAD_CAPTURE_URL =
  "https://crm.konnekteam.kz/api/v1/LeadCapture/ac3da7a407494f0c916fa6c94c7bbe40";
...
export function ContactForm({ defaultService = "" }: { defaultService?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Имя «Иван Петров» → firstName="Иван", lastName="Петров".
    const parts = form.name.trim().split(/\s+/);
    const firstName = parts.shift() ?? "";
    const lastName = parts.join(" ");

    // Услуга + сообщение + источник складываем в Description.
    const description = [
      form.service ? `Услуга: ${form.service}` : null,
      form.message ? `Сообщение: ${form.message}` : null,
      typeof window !== "undefined" ? `Источник: ${window.location.pathname}` : null,
    ].filter(Boolean).join("\n");

    try {
      const res = await fetch(ESPO_LEAD_CAPTURE_URL, {
        method: "POST",
      ...
      setStatus("ok");
      setForm({ name: "", phone: "", company: "", service: defaultService, message: "" });
    } catch (err) {
      console.error("[lead] EspoCRM submit failed:", err);
      setStatus("error");
      setTimeout(() => setStatus("idle"), 8000);
    }
  };

  // Заготовка для WhatsApp (вторичный канал). Текст можно отредактировать в самом WhatsApp.
  const waText = encodeURIComponent(
    `Здравствуйте! Пишу с сайта KONNEKTEAM.\n` +
  ...
  if (status === "ok") {
    return (
      <div style={{
        ...
        textAlign: "center", gap: "16px", padding: "48px 24px",
        ...
        <div style={{ fontFamily: "Montserrat, sans-serif", fontWeight: 700, fontSize: "20px", color: "white" }}>
          Заявка отправлена!
        </div>
        <p style={{ color: "var(--text-secondary)", fontSize: "15px", lineHeight: 1.6, maxWidth: "320px" }}>
          Спасибо за обращение. Наш менеджер свяжется с вами в ближайшее рабочее время.
        </p>
        <button onClick={() => setStatus("idle")}
          style={{
            marginTop: "4px", background: "none", border: "1px solid var(--border)", borderRadius: "6px",
            color: "var(--text-secondary)", fontFamily: "Montserrat, sans-serif", fontSize: "13px",
            fontWeight: 600, padding: "10px 20px", cursor: "pointer", transition: "border-color 0.3s",
          }}
          onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.borderColor = "var(--accent)"}
          onMouseLeave={(e) => (e.currentTarget as HTMLElement).style.borderColor = "var(--border)"}
        >
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <button type="submit" className="btn-glow" disabled={status === "sending"}
        style={{ width: "100%", marginTop: "4px", opacity: status === "sending" ? 0.65 : 1, cursor: status === "sending" ? "wait" : "pointer" }}>
        {status === "sending" ? "Отправляем…" : "Отправить заявку"}
      </button>

      {status === "error" && (
        <p style={{ color: "#ff6b6b", fontSize: "13px", textAlign: "center", lineHeight: 1.5 }}>
          Не удалось отправить заявку. Позвоните нам или напишите в WhatsApp ниже.
        </p>
      )}

      {/* Вторичный канал — WhatsApp (звонки / свободные сообщения) */}
      <a href={`${COMPANY.whatsapp}?text=${waText}`} target="_blank" rel="noopener noreferrer"
        style={{
          ...
          color: "#25D366", fontFamily: "Montserrat, sans-serif", fontWeight: 600, fontSize: "14px",
          textDecoration: "none", transition: "background 0.3s, border-color 0.3s",
        }}
        onMouseEnter={(e) => { const t = e.currentTarget as HTMLElement; t.style.background = "rgba(37,211,102,0.16)"; t.style.borderColor = "#25D366"; }}
        onMouseLeave={(e) => { const t = e.currentTarget as HTMLElement; t.style.background = "rgba(37,211,102,0.08)"; t.style.borderColor = "rgba(37,211,102,0.35)"; }}
      >
